...
- Spec-document tables live in a separate sqlite file (`specdata_database_url`, default sibling `<main-stem>_specdata.db`); `db init/reset/check` take `--scope main|testcase|specdata|all` and `create_schema("specdata")` creates the three relational tables plus the FTS5 (`spec_doc_search`, `spec_doc_search_meta`) and vector (`vec_spec_doc_embeddings`, `vec_spec_doc_meta`) sidecars.
 - Web spec-document portal: `/specs/{spec_id}` links each version to `/specs/{spec_id}/docs?version=...`; the collapsed version page shows the downloaded/parsed state, enqueues `POST /jobs/parse/spec-docs` for parse or force re-parse, and renders the collapsed TOC plus paginated chunks. Search uses a full-width Query field and plural `sections`/`tables` filters; shared `TDoc Search` / `Spec Docs Search` tabs link Spec Docs hits back to version pages and chunk anchors. There is no standalone spec-doc fetch or public repair route; the pre-deployment local corpus is repaired once from cached ZIPs.
