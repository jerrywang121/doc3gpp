...
| `doc3gpp.storage.repositories.search_sql._cover_text` | function | `storage/repositories/search_sql.py:532` | Index-time projection: cover-page fields from `tdoc_cr_cover_page`, falling back to the LS sidecar's `title` / `response_to` / `to_groups` / `cc_groups` (joined to `tdocs` via `ftp_url`) for LS rows |
| `doc3gpp.storage.repositories.vector_sql._build_embed_text` | function | `storage/repositories/vector_sql.py:397` | Index-time embed-text builder; fans out across `tdocs` / `meetings` / `wis` and the three sidecar tables — for LS rows it appends the same `title` / `response_to` / `to_groups` / `cc_groups` fields |
