...
| `extract_changed_functions` | function | `parsers/cr/ttcn_functions.py` | Aggregator: walks `required_changes`, skips entries where either regex returns `None`, deduplicates, and returns `sorted(set(...))` of `"<module_basename>.<function_name>"` entries. |
