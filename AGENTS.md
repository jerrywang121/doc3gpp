...
- Web spec-document portal: `/specs/{spec_id}` links each version to `/specs/{spec_id}/docs?version=...`; the version page shows the downloaded/parsed state, enqueues `POST /jobs/parse/spec-docs` for parse or force re-parse, and renders the TOC plus paginated chunks. Shared `TDoc Search` / `Spec Docs Search` tabs link Spec Docs hits back to version pages and chunk anchors; there is no standalone fetch route.
