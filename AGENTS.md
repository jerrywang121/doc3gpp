...
├── src/doc3gpp/          # package root
 │   ├── cli.py            # Typer commands (12 sub-apps incl. nested TDoc/spec-doc search, 40 commands) + cli_server.py (server sub-app, 6 commands)
...
| Add a spec-document (`spec doc`) source / parse / chunk / search knob | `src/doc3gpp/scraping/spec_doc_source.py` (`resolve_spec_doc_version`, `fetch_spec_doc_zip`) + `src/doc3gpp/parsers/spec_doc.py` (`list_spec_docx`, `order_spec_files`, `extract_spec_toc`) + `src/doc3gpp/parsers/docx_converter.py` (`convert_document_to_blocks`) + `src/doc3gpp/parsers/spec_doc_chunker.py` (`chunk_blocks`) + `src/doc3gpp/services/spec_doc_service.py` (`SpecDocService`) + `src/doc3gpp/services/spec_doc_search_service.py` (`SpecDocSearchService`) + `src/doc3gpp/services/spec_doc_semantic_service.py` (`SpecDocSemanticService`) + `src/doc3gpp/storage/repositories/spec_doc_sql.py` / `spec_doc_search_sql.py` / `spec_doc_vector_sql.py` + `src/doc3gpp/settings/schema.py` (`SpecDocSettings`, TOML `[spec_doc]` block) | Version resolution picks the numeric-newest version with a non-empty `.zip` URL (optional release filter); explicit `--version` stays strict and reports missing ZIP links clearly. Parse fetches when the zip is missing; parse-skip is the immutable `(spec_id, version)` ledger (`parsed_at`, `--force` re-downloads and re-parses); `.doc` entries warn-skip; empty zip → `SpecDocNoDocxError` failure; oversize (`max_zip_size_kb`, default `0` = unlimited) → skip bucket; batch never aborts. Chunking reuses `semantic_search.chunk_size` (default 512) / `chunk_overlap` (default 24, `None` → reuse) + `max_chunk_chars` (default 1500); rows are atomic, oversized-table chunks contain one Markdown header, and overlap never copies table tokens or rows; combined `sections`/`tables` values are newline-delimited single-line identifier/title entries. The dedicated cache defaults to `~/.cache/doc3gpp/specs`; TDoc cache remains `~/.cache/doc3gpp/tdocs`; `chunk_id = {spec_id}@{version}#{chunk_index}`. |
| Add a spec-document CLI command | `src/doc3gpp/cli.py` (`spec_doc_app` / `spec_doc_toc_app` / `spec_doc_search_app`) | `spec doc parse/toc show/search query/search sem/schema` under `spec_app` (`spec doc`); parse prints `ok/skipped/failed` buckets only — its `--format/--output/--compact` flags are accepted but currently ignored. |
| Add a spec-document web route / MCP tool / job | `src/doc3gpp/web/routes/spec_docs.py` (`GET /specs/{spec_id}/docs`, `GET /specs/{spec_id}/docs/toc`, `GET /spec-docs/search`, `GET /spec-docs/search/sem`, `GET /spec-docs/schema`) + `src/doc3gpp/web/routes/jobs.py` (`POST /jobs/parse/spec-docs`) + `src/doc3gpp/web/mcp_server.py` (`get_spec_toc`, `search_spec_docs`, `semantic_search_spec_docs`, `get_spec_doc_schema`, `parse_spec_docs`) + `src/doc3gpp/web/workers/handlers.py` (`_parse_spec_docs`, `JobKind.PARSE_SPEC_DOCS`) | The version-first page shows source state, parse/re-parse job form, TOC, and paginated chunks; search tabs link TDoc and Spec Docs search. No standalone spec-doc fetch route; `JobKind.PARSE_SPEC_DOCS = "parse_spec_docs"` takes `{spec_ids, release?, version?, force}`. |
...
| Add a `server` CLI command | `src/doc3gpp/cli_server.py` | Each subcommand starts with `_require_server_enabled(settings)`; server defaults enabled on `127.0.0.1:13999`; OS install delegates to `web/install.py`. |
| Change server/OS-settings or install helpers | `src/doc3gpp/web/install.py` + `src/doc3gpp/settings/schema.py` (`ServerSettings`/`MCPSettings`) | `[server]`/`[mcp]` TOML blocks; server defaults enabled on loopback port `13999`; server settings are TOML-only (not in the env allowlist). |
...
- `doc3gpp testcase show --testcase <id> [--group <g>]` →
  `TestCaseService.get_all` (every stored group) or `get(id, group)`
...
- `doc3gpp <resource> schema` (`tsg`, `meeting`, `tdoc`, `wi`, `spec`,
  ...
  on `GET /<resources>/schema?format=json` and the `get_*_schema` MCP
...
- `doc3gpp tdoc search index --rebuild-embeddings [--stale-only] [--batch N]
 ...
 - `doc3gpp spec doc parse --spec <id> [--release R] [--version V] [--force]` → `SpecDocService.parse_many` → resolve the numeric-newest `SpecVersion` with a non-empty `.zip` URL via `resolve_spec_doc_version` (after optional release filtering); explicit `--version` is strict and fails clearly when its ZIP link is missing. Fetch the ZIP with internal `fetch_spec_doc_zip` when absent (or use the zip cache at `{spec_doc.cache_dir}/zips/<spec_id>/<version>.zip`), and record the download. `--force` re-downloads before re-parsing, including pairs that already carry `parsed_at`.
 - `doc3gpp spec doc parse --spec <id> [--release R] [--version V] [--force]` → per spec: immutable-skip when `(spec_id, version)` already carries `parsed_at` (unless `--force`) → `parse` = fetch-if-missing + `list_spec_docx` + `convert_document_to_blocks` + `order_spec_files` + `extract_spec_toc` + `chunk_blocks` → `upsert_toc` + `replace_chunks` + per-file markdown cache (`{spec_doc.cache_dir}/markdown/<spec_id>/<version>/`) + `record_parsed` + best-effort auto-index (`upsert_for_version`) / auto-embed (`index_for_version`). Oversized zips → skip bucket; unknown spec/version or empty (no-docx) zips → failure bucket; one spec never aborts the batch (`SpecDocBatchResult(successes/skipped/failures)`). Prints `ok/skipped/failed` buckets only (`--format/--output/--compact` accepted but ignored). Chunk/search metadata uses newline-delimited combined `sections` and `tables`; TOC entries retain `section_no` / `title`.
 ...
 - `doc3gpp spec doc search query "QUERY" [--spec/--release/--version/--sections/--tables] [--limit/--offset/--fields]` → `SpecDocSearchService.search(query, filters)` → FTS5 `MATCH` (built internally via `SearchQueryBuilder`) + rich-filter push-down on the combined `sections` and `tables` columns + `bm25(spec_doc_search, weights)` + one `snippet(...)` per `weight > 0` column (surfaced in `previews` only on a match) → `list[SpecDocHit]` (chunk-level). Weights default `(5.0, 5.0, 5.0, 1.0, 1.0, 1.0)` over `(text, sections, tables, spec_id, version, release)` (`[spec_doc] bm25_weights`).
  - `doc3gpp spec doc search sem QUERY [--fts5-query Q] [--fts5-weight 0.5] [--spec/--release/--version/--sections/--tables] [--limit]` → `SpecDocSemanticService.search` → always embeds `QUERY` (vector path); opt-in FTS5 side feeds `fts5_query` verbatim → fan-out (`limit * fanout_multiplier`) both sides → chunk-level `rrf_merge` (k=60, vector weight `1 - fts5_weight`); without `--fts5-query` pure vector KNN returns dressed as `SpecDocSemanticHit` (`rank_fts5=None`, `hit=None` for vector-only chunks). Requires `[semantic_search].embedding_base_url`; vector-side `spec_id`/`version`/`release` are exact `=`, while `sections`/`tables` use plain `LIKE` (no rich grammar — pass plain values for exact agreement).
 - `doc3gpp spec doc schema` describes the three `spec_doc` tables from the static registry (25 rows; no filters, no DB reads). Chunk rows document `sections` and `tables`; TOC rows continue to document `section_no` and title fields.
- Spec-document tables live in a separate sqlite file (`specdata_database_url`, default sibling `<main-stem>_specdata.db`); `db init/reset/check` take `--scope main|testcase|specdata|all` and `create_schema("specdata")` creates the three relational tables plus the FTS5 (`spec_doc_search`, `spec_doc_search_meta`) and vector (`vec_spec_doc_embeddings`, `vec_spec_doc_meta`) sidecars.
  - Web spec-document portal: `/specs/{spec_id}` links each version to `/specs/{spec_id}/docs?version=...`; the collapsed version page shows the downloaded/parsed state, enqueues `POST /jobs/parse/spec-docs` for parse or force re-parse, and renders the collapsed TOC plus filtered, paginated chunks with a range, up to ten rolling page numbers, ellipses, and first/previous/next/last controls. Search uses a full-width Query field and plural `sections`/`tables` filters; shared `TDoc Search` / `Spec Docs Search` tabs link Spec Docs hits back to version pages and chunk anchors. There is no standalone spec-doc fetch or public repair route; the pre-deployment local corpus is repaired once from cached ZIPs.
