...
async def testcase_schema(
    ...
    ``?format=json`` returns the same payload as
    ``doc3gpp testcase schema --format json``: a bare array of
    ``{table, field, type, nullable, description, values}`` rows from
    :func:`doc3gpp.models.schema_info.schema_payload`. No DB access.
    """
    if format == "json":
        return JSONResponse(content=schema_payload("testcase"))
    tables = RESOURCE_SCHEMAS["testcase"]
    template_name = (
