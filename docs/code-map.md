...
| `TDocLSDetails` | dataclass | `models/tdoc_ls.py` | LS header sidecar (eleven header fields + `variant`, `parser_version`, `extracted_at`). Mirrors the new `tdoc_cr_ls_details` table 1:1. |
| `TDocLSParserResult` | dataclass | `models/tdoc_ls.py` | Parser envelope for LS rows: `cover: TDocLSDetails | None` (`None` when the parser declined). |
| `LSAttachment` | TypedDict | `models/tdoc_ls.py` | One `Attachments:` row (`doc_number` + `description`); stored gzip-JSON on the table. |
...
| `TDocShowRecord` | dataclass | `models/tdoc_show.py` | Composite render record for `tdoc show --tdoc`: `TDoc` + slim cover + optional TTCN + optional body-change sidecar + optional LS sidecar + `extracted_at` + `TDocFile` list. |
| `TDocShowRecordByUrl` | dataclass | `models/tdoc_show.py` | URL-anchored variant of `TDocShowRecord` for `tdoc show --ftp-url`. |
| `TDocShowRepos` | dataclass | `models/tdoc_show.py` | Bundle of repos (`tdoc`, `cr`, `cr_ttcn`, `cr_change_details`, `file`, `ls`) consumed by `TDocShowRecord.from_tdoc_id`. |
...
| `LSParserRepository` | Protocol | `repository/protocols.py` | Contract for the LS sidecar storage (`tdoc_cr_ls_details`, one row per immutable `ftp_url`): `upsert`, `get_by_url`, `get_by_tdoc_id`, `get_by_variant`. Callers holding a full upstream URL must `normalize_ftp_path` first. |
...
| `TDocCrService` | class | `services/tdoc_cr_service.py` | End-to-end CR extraction (zip → cache → python-docx → parse → persist). Constructor takes `cr_ttcn_repository`; fans `TDocCRParseResult(cover, ttcn)` out across the slim cover-page repo, the optional TTCN sidecar repo, and the `tdoc_extracts` repo in three independent upserts. Also exposes `extract_from_url` / `extract_from_bytes` for the `tdoc parse --from-path/--from-url` direct-mode path, and the public `collect_3gpp_file_urls(url, *, max_depth)` alias (same body as the private `_collect_3gpp_file_urls`) used by the auto-sync URL-candidate helper. LS dispatch: `_resolve_parser(tdoc_id, tdoc_type, source)` returns an `LSParserBase` variant for `tdoc_type == "LS"` rows; DB-mode `extract()` dispatches LS rows through that `LSParserBase` variant, and the LS branch of `extract()` writes the `tdoc_cr_ls_details` sidecar + the `tdoc_extracts.cache_file` metadata row (no cover / TTCN / change sidecars) — the same `parse_ls` + upsert path the direct-mode `extract_from_bytes` (raw-markdown mode) and `_extract_from_3gpp_url` branches use via `ls_repository` (constructor-injected, default `SQLAlchemyLSParserRepository`). |
| `build_ls_repository` | function | `services/factory.py:102` | Factory for `LSParserRepository`; wired into `build_tdoc_cr_service` (factory.py:254) and the `tdoc show` / MCP / web read paths. |
...
| `build_*` | helpers | `services/factory.py` | Factory used by the CLI to wire repo / service instances. `build_spec_service` injects `SQLAlchemySpecRepository` + the `sync.spec_sync_interval` setting; `build_ls_repository` constructs the LS sidecar repo. |
...
| `TDocParser` Protocol / `TDocParserRegistry` / `build_default_registry` | Protocol / class / function | `parsers/tdoc_parsers.py` | `TDocParser.parse()` returns a `TDocCRParseResult` (cover + optional TTCN sidecar); `parse_ls()` returns `TDocLSParserResult` (LS-family; the base raises `NotImplementedError`). `build_default_registry` registers `ThreeGPPLSParser` (wins for `tdoc_type='LS'` rows), `TTCNCRParser`, then the generic `CRParser`. |
| `LSParserBase` | class | `parsers/ls/ls_parsers.py` | LS parser orchestrator: header detection via `is_ls_header_present`, cover extraction via an injected variant `LSCoverPageParser`, assembles `TDocLSDetails`. `parse()` raises `NotImplementedError` — the service always calls `parse_ls()` for LS rows. |
| `ThreeGPPLSParser` | class | `parsers/ls/variants/three_gpp.py` | v1 LS variant: binds `LSCoverPageParser`, stamps `VARIANT = "3gpp"`; registered in `build_default_registry`. |
| `IEEELSParser` / `ETSILSParser` | classes | `parsers/ls/variants/ieee.py` / `parsers/ls/variants/etsi.py` | v2 stubs — **not** registered in the default registry; placeholder cover extractors raise `NotImplementedError` on `parse_ls`. |
| `LSCoverPageParser` | class | `parsers/ls/cover_page.py` | 3GPP LS header extractor (title, response-to triple, release, work-item pair, source, to / cc groups, attachments). `supports_source` is the broad catch-all in v1 (`None` or any non-blank source). |
| `is_ls_header_present` / `LSHeaderMissingError` | function / exception | `parsers/ls/header.py` | Probe the markdown head (first 100 lines) for the tabbed `Meeting` / `TDoc` first line, a `Title:` starting with `LS on`, and at least one `Source:` / `To:` / `Cc:` cell. `LSHeaderMissingError(snippet)` carries the scanned blob for error messages. |
...
| ORM classes | `Mapped[]` | `storage/db/models.py` | `TDocORM`, `MeetingORM`, `TsgORM`, `WiORM`, `TDocFileORM`, slim `TDocCrDetailOrm` (cover-page only), `TDocCrTtcnDetailOrm` (TTCN sidecar), `TDocCrLSDetailOrm` (LS sidecar, `models.py:296`), `TDocCrChangeDetailOrm` (body-change sidecar), `TDocExtractOrm` (cache metadata: `cache_file` String(255), indexed), `SpecORM` (header table keyed by `spec_id`), `SpecVersionORM` (one row per `(spec_id, version)` with `ftp_url`, `pdf_url`, `crs`, etc.). |
...
| `compress_json` / `decompress_json` | functions | `storage/compression.py` | Shared gzip JSON helpers used by `SQLAlchemyTDocCrRepository`, `SQLAlchemyTDocCrTtcnRepository`, and `SQLAlchemyLSParserRepository` for any binary JSON detail column (the TTCN sidecar's `required_changes` and the LS sidecar's `attachments_json` today). `decompress_json` is tolerant — `None` / empty / gzip / JSON / Unicode errors all resolve to `None` plus a warning; legacy uncompressed blobs decode transparently. |
...
| `SQLAlchemyLSParserRepository` | class | `storage/repositories/tdoc_cr_ls_sql.py` | SQL impl of `LSParserRepository` for the `tdoc_cr_ls_details` sidecar. One row per immutable `ftp_url` (PK) + FK `tdoc_id` (CASCADE); `attachments_json` gzip-compressed via `storage/compression.py`; `variant` defaults to `"3gpp"`. Same lazy-bootstrap contract as the TTCN repo. |
...
| `doc3gpp.storage.repositories.search_sql._cover_text` | function | `storage/repositories/search_sql.py:532` | Index-time projection: cover-page fields from `tdoc_cr_cover_page`, falling back to the LS sidecar's `title` / `response_to_title` / `to_groups` / `cc_groups` (joined to `tdocs` via `ftp_url`) for LS rows |
| `doc3gpp.storage.repositories.vector_sql._build_embed_text` | function | `storage/repositories/vector_sql.py:397` | Index-time embed-text builder; fans out across `tdocs` / `meetings` / `wis` and the three sidecar tables — for LS rows it appends the same `title` / `response_to_title` / `to_groups` / `cc_groups` fields |
