...
| GET | `/testcases/{testcase_id}` | Testcase detail (HTML or JSON). JSON is `{"testcase":{...},"statuses":[...]}`; unknown id → 404 `testcase_not_found`. |
...
(`statuses` stays a dict). `get_testcase(testcase_id)` calls
`services.testcase.get(testcase_id)`; `None` raises
`TestcaseNotFoundError` (MCP `-32004`); else the payload is
`{"testcase": {8 header fields}, "statuses":
